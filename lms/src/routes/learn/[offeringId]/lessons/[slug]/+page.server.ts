...
export async function load({ params, platform }: { params: Record<string, string>; platform: App.Platform }) {
	...
	try {
		...
		const offering = await cachedDbFirst(db, 'SELECT * FROM course_offerings WHERE id = ?', [offeringId]);
		if (!offering) throw error(404, 'Course offering not found');

		const course = await cachedDbFirst(db, 'SELECT * FROM courses WHERE id = ?', [offering.course_id]);

		// Find lesson via content_blocks tree (type='lesson')
		let lessonBlock = await cachedDbFirst(
			db,
		...
		if (lessonBlock) {
			...
			const allBlocks = await cachedDbQuery(db,
