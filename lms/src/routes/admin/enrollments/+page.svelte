<script lang="ts">
	...
	import { Button, Select, Textarea, Badge, Table, TableHeader, TableBody, TableRow, TableCell, TableHead, EmptyState } from '$lib/components/ui';
	...
	const offeringOptions = $derived(
		offerings.map(o => ({ value: o.id, label: `${o.name} (${o.code || 'no code'})` }))
	);
...
<div class="bulk-enroll-page">
	...
	<div class="card">
		...
		{:else}
			<Select id="offering-select" options={[{ value: '', label: '-- Select offering --' }, ...offeringOptions]} bind:value={selectedOfferingId} />
		{/if}
	</div>
	...
	<div class="mode-toggle">
		<Button
			variant={inputMode === 'csv' ? 'primary' : 'secondary'}
		...
		<div class="card">
			...
			{:else}
				...
				<div class="preview-actions">
					<Button variant="secondary" size="sm" onclick={generatePreview}>👁️ Preview Emails</Button>
				</div>
			{/if}
		</div>
	{/if}
	...
	{#if inputMode === 'paste'}
		<div class="card">
			...
			/>
			<div class="preview-actions">
				<Button variant="secondary" size="sm" onclick={generatePreview} disabled={!emailsText.trim()}>
					👁️ Preview Emails
				</Button>
			</div>
		</div>
	...
	{#if showPreview}
		<div class="card preview-card">
			<div class="preview-header">
				<h3>Preview</h3>
				<Button variant="ghost" size="sm" onclick={hidePreview}>✕</Button>
			</div>
					...
					<TableBody>
						{#each previewEmails.slice(0, 50) as entry, i}
							<TableRow>
								...
								<TableCell>
									<Badge variant={entry.valid ? 'success' : 'default'}>{entry.valid ? '✓' : '✗'}</Badge>
								</TableCell>
							</TableRow>
						{/each}
					</TableBody>
					...
					<TableBody>
						{#each results as r, i}
							<TableRow>
								...
								<TableCell>
									<Badge variant={r.status === 'created' ? 'success' : r.status === 'error' ? 'danger' : 'default'}>
										{r.status === 'created' ? '✅ Created' : r.status === 'skipped' ? '⏭️ Skipped' : '❌ Error'}
									</Badge>
								</TableCell>
								<TableCell class="reason-cell">{r.reason || '-'}</TableCell>
							</TableRow>
						{/each}
					</TableBody>
				</Table>
...
<style>
	.bulk-enroll-page { max-width: 800px; }
	...
	.drop-hint code { background: var(--bg-secondary); padding: 1px 4px; border-radius: 3px; }
	...
	.emails-textarea { font-family: monospace; font-size: 13px; }
	.hint { font-size: 12px; color: var(--text-secondary); margin-bottom: 8px; }

	/* Preview */
	.preview-card { border-color: var(--accent); }
	.preview-header {
		display: flex;
		justify-content: space-between;
		align-items: center;
	...
	.submit-card { background: var(--surface); }
	.error-msg {
		font-size: 13px;
	...
	/* Results */
	.results-card h2 { font-size: 18px; margin: 0 0 16px 0; }
	.summary-stats {
		display: flex;
		gap: 12px;
		margin-bottom: 16px;
	...
	.stat-label { font-size: 12px; color: var(--text-secondary); text-transform: uppercase; letter-spacing: 0.04em; }
	.reason-cell { color: var(--text-secondary); font-size: 12px; }
