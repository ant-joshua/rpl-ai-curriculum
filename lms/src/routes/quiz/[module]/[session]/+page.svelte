<script lang="ts">
  ...
  onMount(() => {
    ...
    if (typeof localStorage !== 'undefined') {
      const tryKey = `quiz-tries-${moduleSlug}-${sessionId}`;
      const bestKey = `quiz-best-${moduleSlug}-${sessionId}`;
...
<style>
  ...
  .score-pct { font-size: 16px; color: var(--text-secondary); margin-bottom: 16px; }
