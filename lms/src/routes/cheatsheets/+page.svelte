<script lang="ts">
	...
	onMount(async () => {
		try {
			const res = await fetch('/content/references/cheatsheets.json');
			if (!res.ok) throw new Error('Gagal memuat cheatsheets');
			const json = await res.json();
			items = json.items || [];
		} catch (e) {
			errorMsg = 'Gagal memuat data cheatsheets';
		} finally {
			loading = false;
		}
	});
...
<div class="reference-page">
	<header class="page-header">
		<h1>📝 Cheatsheets</h1>
		<p class="page-desc">Ringkasan cepat sintaks, konsep, dan perintah penting — dari JavaScript, TypeScript, Git, hingga database dan arsitektur web.</p>
	</header>
		...
		<main class="content-area">
			...
			{:else}
				<div class="placeholder">
					<p>Pilih cheatsheet dari daftar di samping untuk melihat ringkasan.</p>
				</div>
			{/if}
		</main>
	</div>
