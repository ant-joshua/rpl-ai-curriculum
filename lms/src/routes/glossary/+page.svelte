<script lang="ts">
	...
	onMount(async () => {
		try {
			const res = await fetch('/content/references/glossary.json');
			if (!res.ok) throw new Error('Gagal memuat glossary');
	...
	<div class="layout">
		<aside class="sidebar-nav">
			...
			{#if filteredItems.length === 0 && !loading}
				<EmptyState icon="🔍" title={t('glossary.no_results')} description="Try adjusting your search." />
			{/if}
		</aside>
