...
export async function GET({ url, platform, locals }: { url: URL; platform: App.Platform; locals: any }) {
	try {
		...
		const dosenId = url.searchParams.get('dosen_id') || undefined;
		const data = await repo.getKelasKuliah({ academicSemesterId: semesterId, dosenId });
...
export async function POST({ request, platform, locals }: { request: Request; platform: App.Platform; locals: any }) {
	try {
		...
		if (!body.course_catalog_id || !body.academic_semester_id || !body.code || !body.dosen_id) {
			throw error(400, 'course_catalog_id, academic_semester_id, code, dan dosen_id wajib diisi');
		}
		const data = await repo.createKelasKuliah(body);
