...
export async function GET({ params, platform }: { params: { slug: string }; platform: App.Platform }): Promise<Response> {
	try {
		...
		const row = await db.prepare('SELECT * FROM admin_content WHERE key = ? AND type = ?').bind(params.slug, 'project').first<any>();
...
export async function PUT({ request, params, platform }: { request: Request; params: { slug: string }; platform: App.Platform }): Promise<Response> {
	try {
		const db = getDB(platform);
		const existing = await db.prepare('SELECT * FROM admin_content WHERE key = ? AND type = ?').bind(params.slug, 'project').first<any>();
		...
		return jsonResponse({ success: true, data: { id: existing.id, key: params.slug, ...merged } });
	} catch (e: unknown) {
		const msg = e instanceof Error ? e.message : 'Unknown error';
		return jsonResponse({ success: false, error: msg }, 500);
	}
...
export async function DELETE({ params, platform }: { params: { slug: string }; platform: App.Platform }): Promise<Response> {
	try {
		const db = getDB(platform);
		await db.prepare('DELETE FROM admin_content WHERE key = ? AND type = ?').bind(params.slug, 'project').run();
