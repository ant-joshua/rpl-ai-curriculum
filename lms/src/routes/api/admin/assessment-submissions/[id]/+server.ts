...
export async function GET({ params, platform }: { params: { id: string }; platform: App.Platform }): Promise<Response> {
	try {
		...
		const row = await db.prepare(
			'SELECT asub.*, u.display_name AS user_name FROM assessment_submissions asub LEFT JOIN users u ON u.id = asub.user_id WHERE asub.id = ?'
...
export async function PUT({ request, params, platform }: { request: Request; params: { id: string }; platform: App.Platform }): Promise<Response> {
	try {
		const db = getDB(platform);
		const existing = await db.prepare('SELECT * FROM assessment_submissions WHERE id = ?').bind(params.id).first<any>();
		if (!existing) return jsonResponse({ success: false, error: 'Not found' }, 404);

		const body = await request.json();

		await db.prepare(
			`UPDATE assessment_submissions SET status = ?, answers = ?, score = ?, max_score = ?, graded_by = ?, graded_at = ?, feedback = ?, submitted_at = ?, updated_at = datetime('now') WHERE id = ?`
		).bind(
			body.status ?? existing.status,
			body.answers ? JSON.stringify(body.answers) : existing.answers,
		...
		const updated = await db.prepare('SELECT * FROM assessment_submissions WHERE id = ?').bind(params.id).first<any>();
