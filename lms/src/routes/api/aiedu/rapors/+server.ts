...
// GET /api/aiedu/rapors — list rapor history
export async function GET({ request, platform }: { request: Request; platform: App.Platform }): Promise<Response> {
	try {
		...
		const { results } = await db.prepare(
			'SELECT id, student_name, subject, grade, created_at FROM aiedu_rapors WHERE user_id = ? ORDER BY created_at DESC LIMIT 20'
