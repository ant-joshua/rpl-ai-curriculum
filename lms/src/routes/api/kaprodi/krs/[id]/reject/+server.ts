...
export async function POST({ params, request, platform, locals }: { params: { id: string }; request: Request; platform: App.Platform; locals: any }) {
	try {
		...
		const rejected = await repo.rejectKRS(params.id);
		if (!rejected) throw error(404, 'KRS tidak ditemukan atau status bukan submitted');
		return json({ success: true, message: 'KRS ditolak, status diubah ke revision' });
	} catch (e: unknown) {
