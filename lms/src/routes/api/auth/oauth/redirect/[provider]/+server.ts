...
export async function GET({ request, platform, params }: {
	...
	if (provider === 'google') {
		...
		const clientId = getSecret(platform, 'OAUTH_MICROSOFT_CLIENT_ID') || 'MICROSOFT_CLIENT_ID_PLACEHOLDER';
		authUrl = `${MICROSOFT_AUTH_URL}?client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&scope=openid%20email%20profile%20offline_access&state=${state}`;
	} else {
