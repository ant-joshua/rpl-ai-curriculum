...
export async function GET({ url, platform, locals }: { url: URL; platform: App.Platform; locals: any }) {
	...
	try {
		...
		// Get existing PH scores for the specified KD
		const existingScores = await db.prepare(`
			SELECT * FROM k13_ph
			WHERE class_subject_id = ? AND kompetensi_dasar_id = ? AND semester = ? AND tenant_id = ?
		`).bind(classSubjectId, kdId, semester, tenantId).all<any>();

		// Build map of existing scores by user_id
		const scoreMap: Record<string, any> = {};
		for (const row of (existingScores.results || [])) {
			scoreMap[row.user_id] = row;
		}

		// Attach existing scores to students
