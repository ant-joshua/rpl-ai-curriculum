...
export const load: PageServerLoad = async ({ request, platform, url }) => {
	...
	if (offeringIds.length > 0) {
		...
		const { results: counts } = await cachedDbQuery<{ course_offering_id: string; cnt: number }>(
			db,
	...
	if (userId && offeringIds.length > 0) {
		const placeholders = offeringIds.map(() => '?').join(',');
		const { results: enrolled } = await cachedDbQuery<{ course_offering_id: string }>(
			db,
	...
	if (courseIds.length > 0) {
		...
		const { results: prereqs } = await cachedDbQuery<any>(
			db,
	...
	if (userId) {
		const { results: completed } = await cachedDbQuery<{ course_id: string }>(
			db,
