...
<Select bind:value={typeFilter} onchange={loadFeeStructures} options={feeTypes.map((ft) => ({ value: ft.value, label: ft.label }))} />
	...
	{:else if error}
		<div class="pg-error-block">
			<p>{error}</p>
			<Button variant="ghost" class="pg-btn pg-" onclick={loadFeeStructures}>{t('common.retry')}</Button>
		</div>
...
<Input bind:value={formAcademicYear} placeholder="Contoh: 2025/2026" />
						</div>
						<div class="pg-field">
							<label class="pg-label">{t('admin.semester')}</label>
<Input bind:value={formSemester} placeholder="Contoh: 1" />
						</div>
						<div class="pg-field pg-field-full">
							<label class="pg-label">{t('common.description')}</label>
<Input bind:value={formDescription} placeholder="Deskripsi opsional" />
						</div>
						<div class="pg-field pg-field-full">
				...
				<div class="pg-modal-footer">
					...
					<Button variant="primary" class="pg-btn pg-" onclick={submitFee} disabled={saving || !formName || formAmount <= 0}>
...
<style>
	...
	.pg-title { font-size: 24px; font-weight: 600; color: var(--text-primary, var(--text)); margin: 0; letter-spacing: -0.3px; }
	.pg-subtitle { font-size: 13px; color: var(--text-secondary, var(--text-secondary)); margin: 4px 0 0; }

	.pg-filters { display: flex; align-items: center; gap: 12px; }
	.pg-filter-count { font-size: 13px; color: var(--text-secondary, var(--text-secondary)); }
	.pg-select {
	...
	.pg-loading { display: flex; flex-direction: column; align-items: center; gap: 12px; padding: 60px 20px; color: var(--text-secondary, var(--text-secondary)); }
	.pg-spinner { width: 32px; height: 32px; border: 3px solid var(--border-color, rgba(0,0,0,0.06)); border-top-color: var(--accent, var(--accent)); border-radius: 50%; animation: pg-spin 0.7s linear infinite; }
	@keyframes pg-spin { to { transform: rotate(360deg); } }

	.pg-error-block { display: flex; flex-direction: column; align-items: center; gap: 12px; padding: 40px; color: var(--danger); text-align: center; background: rgba(239,68,68,0.05); border: 1px solid rgba(239,68,68,0.15); border-radius: 10px; }
	.pg-empty { display: flex; flex-direction: column; align-items: center; gap: 12px; padding: 60px 20px; color: var(--text-secondary, var(--text-secondary)); }
	...
	.pg-fee-name { font-size: 15px; font-weight: 600; color: var(--text-primary, var(--text)); margin: 4px 0 0; }
	.pg-fee-code { font-size: 12px; font-family: 'SF Mono', 'Fira Code', monospace; color: var(--text-secondary, var(--text-secondary)); }
	.pg-fee-amount { font-size: 22px; font-weight: 700; color: var(--accent, var(--accent)); letter-spacing: -0.5px; margin: 4px 0; }
	.pg-fee-desc { font-size: 12px; color: var(--text-secondary, var(--text-secondary)); margin: 0; line-height: 1.4; }
	.pg-fee-meta { display: flex; gap: 8px; flex-wrap: wrap; margin-top: 4px; }
	.pg-fee-meta span { font-size: 11px; color: var(--text-secondary, var(--text-secondary)); background: rgba(0,0,0,0.04); padding: 2px 8px; border-radius: 4px; }

	.pg-badge { display: inline-block; padding: 3px 10px; border-radius: 20px; font-size: 11px; font-weight: 600; }
	.pg-badge-type { background: rgba(113,112,255,0.12); color: var(--accent); }
	.pg-badge-off { background: rgba(156,163,175,0.12); color: var(--text-muted); margin-left: 4px; }
	...
	.pg-modal { background: var(--text); border: 1px solid var(--border-color, rgba(255,255,255,0.1)); border-radius: 12px; width: 100%; max-width: 500px; max-height: 90vh; display: flex; flex-direction: column; }
	.pg-modal-header { display: flex; align-items: center; justify-content: space-between; padding: 16px 20px; border-bottom: 1px solid var(--border-color, rgba(0,0,0,0.06)); }
	.pg-modal-header h2 { font-size: 16px; font-weight: 600; color: var(--text-primary, var(--text)); margin: 0; }
	.pg-modal-body { padding: 20px; overflow-y: auto; flex: 1; }
	.pg-modal-footer { display: flex; justify-content: flex-end; gap: 10px; padding: 16px 20px; border-top: 1px solid var(--border-color, rgba(0,0,0,0.06)); }

	/* Form */
	.pg-form-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }
	.pg-field-full { grid-column: 1 / -1; }
	.pg-label { display: block; font-size: 12px; font-weight: 500; color: var(--text-secondary, var(--text-secondary)); margin-bottom: 5px; }
	.pg-required { color: var(--danger); }
	.pg-input {
		width: 100%;
		padding: 8px 12px;
		font-size: 13px;
	...
	.pg-input:focus { border-color: var(--accent, var(--accent)); }
	.pg-toggle-label { display: flex; align-items: center; gap: 8px; font-size: 13px; color: var(--text-primary, var(--border)); cursor: pointer; }
	.pg-toggle-input { accent-color: var(--accent, var(--accent)); }
	...
	.pg-btn-primary { background: var(--accent, var(--accent)); color: white; }
	.pg-btn-primary:hover { opacity: 0.9; }
	.pg-btn-primary:disabled { opacity: 0.5; cursor: not-allowed; }
	.pg-btn-ghost { background: transparent; color: var(--accent, var(--accent)); border: 1px solid var(--border-color, rgba(255,255,255,0.1)); }
	.pg-btn-ghost:hover { background: rgba(0,0,0,0.04); }
	.pg-btn-sm { padding: 5px 12px; font-size: 12px; }
	.pg-btn-icon { display: inline-flex; align-items: center; justify-content: center; width: 28px; height: 28px; border-radius: 6px; background: transparent; border: none; color: var(--text-secondary, var(--text-secondary)); cursor: pointer; transition: all 0.15s; }
	.pg-btn-icon:hover { background: rgba(0,0,0,0.08); color: var(--text-primary, var(--text)); }
</style>
