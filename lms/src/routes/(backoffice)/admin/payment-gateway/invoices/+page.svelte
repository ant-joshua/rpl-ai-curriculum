		...
		},
		{
			...
			cell: ({ getValue }) => {
				...
				const colors: Record<string, { bg: string; color: string }> = {
					unpaid: { bg: 'rgba(245,158,11,0.12)', color: 'var(--warning)' },
					partial: { bg: 'rgba(245,158,11,0.12)', color: 'var(--warning)' },
					paid: { bg: 'rgba(34,197,94,0.12)', color: 'var(--success)' },
					overdue: { bg: 'rgba(239,68,68,0.12)', color: 'var(--danger)' },
					cancelled: { bg: 'rgba(138,143,152,0.12)', color: 'var(--text-secondary)' },
				};
		...
		},
		{
			...
			cell: ({ row }) => {
				...
				if (inv.status !== 'paid' && inv.status !== 'cancelled') {
					html += `<button onclick="window.__openPaymentModal('${inv.id}')" style="padding:5px 12px;background:rgba(113,112,255,0.12);color:var(--accent);border:none;border-radius:6px;font-size:12px;cursor:pointer">Bayar</button>`;
				}
...
<style>
	...
	.badge-unpaid { background: rgba(245,158,11,0.12); color: var(--warning); }
	.badge-partial { background: rgba(245,158,11,0.12); color: var(--warning); }
	.badge-paid { background: rgba(34,197,94,0.12); color: var(--success); }
	.badge-overdue { background: rgba(239,68,68,0.12); color: var(--danger); }
	.badge-cancelled { background: rgba(138,143,152,0.12); color: var(--text-secondary); }
	...
	.pg-page-info { font-size: 13px; color: var(--text-secondary, var(--text-secondary)); }
	...
	.pg-modal { background: var(--text); border: 1px solid var(--border-color, rgba(255,255,255,0.1)); border-radius: 12px; width: 100%; max-width: 560px; max-height: 90vh; display: flex; flex-direction: column; }
	...
	.pg-detail-label { font-size: 11px; color: var(--text-secondary, var(--text-secondary)); font-weight: 500; text-transform: uppercase; letter-spacing: 0.03em; }
	.pg-detail-value { font-size: 14px; color: var(--text-primary, var(--border)); }

	.pg-subsection { font-size: 14px; font-weight: 600; color: var(--text-primary, var(--text)); margin: 20px 0 10px; }
	.pg-notes { font-size: 13px; color: var(--text-secondary, var(--text-secondary)); margin-top: 16px; }
	...
	/* Payment Summary */
	.pg-pay-summary { padding: 12px 16px; background: rgba(113,112,255,0.08); border-radius: 8px; margin-bottom: 16px; font-size: 14px; color: var(--text-primary, var(--border)); }
	...
	.pg-btn-accent { background: rgba(113,112,255,0.12); color: var(--accent); }
	.pg-btn-accent:hover { background: rgba(113,112,255,0.2); }
	.pg-btn-sm { padding: 5px 12px; font-size: 12px; }
	.pg-btn-icon { display: inline-flex; align-items: center; justify-content: center; width: 32px; height: 32px; border-radius: 6px; background: transparent; border: none; color: var(--text-secondary, var(--text-secondary)); cursor: pointer; transition: all 0.15s; }
	.pg-btn-icon:hover { background: rgba(0,0,0,0.08); color: var(--text-primary, var(--text)); }
	.pg-btn-danger:hover { color: var(--danger); }
</style>
