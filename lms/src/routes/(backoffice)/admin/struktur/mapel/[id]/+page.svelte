<script lang="ts">
	...
	async function loadData() {
		...
		try {
			const [mapelRes, kdRes] = await Promise.all([
				fetch(`/api/admin/struktur/mapel/${mapelId}`),
				fetch(`/api/admin/struktur/mapel/${mapelId}/kd`),
			]);
	...
	async function submitForm() {
		...
		try {
			const res = await fetch(`/api/admin/struktur/mapel/${mapelId}/kd`, {
				method: 'POST',
	...
	async function deleteKD(kdId: string) {
		...
		try {
			const res = await fetch(`/api/admin/struktur/mapel/${mapelId}/kd/${kdId}`, { method: 'DELETE' });
			const json = await res.json();
	...
	const typeColors: Record<string, string> = {
		pengetahuan: 'background: rgba(99,102,241,0.1); color: #6366f1',
	...
	const compTypeColors: Record<string, string> = {
		'3': 'background: rgba(99,102,241,0.1); color: #6366f1',
...
<div class="page">
	...
	{:else if error}
		<div class="error-state">
			...
			<a href="/admin/struktur/mapel" class="btn-primary">← Kembali</a>
		</div>
	{:else if mapel}
		<div class="header">
			<div>
				<a href="/admin/struktur/mapel" class="back-link">← Mata Pelajaran</a>
				<h1>{mapel.name}</h1>
				<div class="meta">
...
<style>
	...
	.kd-code { font-weight: 600; background: rgba(99,102,241,0.1); color: #6366f1; }
