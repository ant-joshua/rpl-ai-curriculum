<script lang="ts">
	...
	async function loadData() {
		...
		try {
			const [assignRes, kelasRes, mapelRes, guruRes] = await Promise.all([
				fetch('/api/admin/struktur/guru-mapel'),
				fetch('/api/admin/struktur/kelas'),
				fetch('/api/admin/struktur/mapel'),
				fetch('/api/admin/struktur/guru'),
			]);
	...
	async function submitForm() {
		...
		try {
			const res = await fetch('/api/admin/struktur/guru-mapel', {
				method: 'POST',
	...
	async function deleteAssignment(id: string) {
		...
		try {
			const res = await fetch(`/api/admin/struktur/guru-mapel/${id}`, { method: 'DELETE' });
			const json = await res.json();
