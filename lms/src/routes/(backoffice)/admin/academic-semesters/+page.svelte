	...
	const columns: ColumnDef<any, any>[] = [
		...
		},
		{
			...
			cell: ({ getValue }) => {
				...
				if (active) {
					return '<span style="display:inline-block;padding:2px 8px;border-radius:6px;font-size:11px;font-weight:600;background:rgba(16,185,129,0.1);color:var(--success)">'+t('common.active')+'</span>';
				}
		...
		},
		{
			...
			cell: ({ getValue, row }) => {
				...
				return `<button onclick="window.__semSetActive('${id}')" style="padding:4px 10px;border:1px solid rgba(16,185,129,0.3);border-radius:6px;background:rgba(16,185,129,0.1);color:var(--success);font-size:12px;cursor:pointer">Set Aktif</button>`;
			}
		}
	];
...
<style>
	...
	.modal { background: var(--surface); border: 1px solid var(--border); border-radius: 14px; width: 100%; max-width: 520px; box-shadow: 0 0 0 1px var(--border); }
