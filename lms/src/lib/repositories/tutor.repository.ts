...
export class TutorRepository {
	...
	async getPackages(opts: { status?: string; type?: string } = {}): Promise<LearningPackageRow[]> {
		let sql = `SELECT * FROM learning_packages WHERE tenant_id = ?`;
		const params: any[] = [this.tenantId];
		if (opts.status) { sql += ' AND status = ?'; params.push(opts.status); }
		if (opts.type) { sql += ' AND type = ?'; params.push(opts.type); }
		sql += ' ORDER BY created_at DESC';
		const rows = await this.db.prepare(sql).bind(...params).all<LearningPackageRow>();
		return rows.results || [];
	}
	...
	async getEnrollments(opts: { batchId?: string; status?: string } = {}): Promise<EnrollmentRow[]> {
		let sql = `
	...
	async getBillings(opts: {
		...
		if (opts.userId) { sql += ' AND br.user_id = ?'; params.push(opts.userId); }
		if (opts.status) { sql += ' AND br.status = ?'; params.push(opts.status); }
		if (opts.batchId) { sql += ' AND br.batch_id = ?'; params.push(opts.batchId); }
